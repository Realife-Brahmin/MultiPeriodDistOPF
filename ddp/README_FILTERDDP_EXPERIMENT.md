...
| 6a — grid-power bounds | **PASS** | [results/copper_plate/stage6_output.log](results/copper_plate/stage6_output.log): 14 iters, status 0, `\|ΔJ\| 3.7e-11`, active set `Psub[2]≤1.35` matches reference | none |
...
| 6d — all bounds together, T = 3 | **PASS** | same log: 16 iters, status 0, `\|ΔJ\| 7.3e-11`, active set `B[2]≤2.05` + `Psub[2]≤1.45` matches | none |
...
# environment
julia --startup-file=no --project=ddp/env -e 'using Pkg; Pkg.instantiate()'
...
# Stages 5 and 6
julia --startup-file=no --project=ddp/env ddp/examples/power_system/copper_plate_battery.jl
julia --startup-file=no --project=ddp/env ddp/examples/power_system/copper_plate_battery_bounds.jl
```
