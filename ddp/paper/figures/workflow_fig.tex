...
\node[anchor=south, text=cDual!70!black, inner sep=1pt] at (7.5,0.52)
  {duals per stage: $\varphi^t_{\text{bal}}, \varphi^t_{\text{soc}}$ (equalities),
   $\nu^t, \lambda^t$ (bounds)};

% ---------- the tape ----------
\node[st]  (B1) at (0.00,0) {$B^1$};
\node[stg] (S1) at (2.50,0) {$t\!=\!1$\\[-2pt]$c^1, p_L^1$};
\node[st]  (B2) at (5.00,0) {$B^2$};
\node[stg] (S2) at (7.50,0) {$t\!=\!2$\\[-2pt]$c^2, p_L^2$};
\node[st]  (B3) at (10.00,0) {$B^3$};
\node[stg] (S3) at (12.50,0) {$t\!=\!3$\\[-2pt]$c^3, p_L^3$};
\node[st]  (B4) at (15.00,0) {$B^4$};
...
\node[anchor=north, text=cPrim!80!black, inner sep=1pt] at (7.5,-0.40)
  {controls per stage: $P_{\text{Subs}}^t,\, P_B^t,\, s^t$ \;
   ($B^1\!=\!B_0$ fixed, $B^4$ the terminal energy)};
...
\end{tikzpicture}
\caption{How the copper-plate problem is actually solved, drawn at $T = 3$ so the
tape fits; the structure is the same at any horizon, and the results reported
here are $T = 6$.  Read it as one worked example.  Each stage owns its own price
and demand, so nothing is interpolated and the state is the stored energy alone
--- which is why the cost-to-go derivatives $V_B$ and $V_{BB}$ carried backward
along the top are single numbers rather than a vector and a matrix.  The backward
sweep builds a local model at each stage and leaves behind an affine rule; the
forward sweep applies those rules left to right at one step size $\gamma$, moving
the powers, the equality multipliers and the bound multipliers together.  The
stage solve is the interesting part: three controls against two equalities leave
exactly one direction free, so the ``dense'' factorisation is a division by a
single number, and the regularisation DDP is known for fires only if that number
turns non-positive --- which on this problem it never did.}
\label{fig:workflow}
\end{figure*}
