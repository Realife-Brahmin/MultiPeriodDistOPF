...
The state is the stored energy and a counter, and the counter is not a degree of
freedom --- $\tau^t = t$ always --- so $B^t$ is the only state that varies:
\begin{equation}
...
\begin{alignat}{3}
&\varphi_{\text{bal}}^t   &\;\;:\;\;& P_{\text{Subs}}^t + P_B^t - p_L(\tau^t) = 0
...
second derivative of $f$ and of $c$ with respect to the controls vanishes.  What
remains is
\begin{alignat}{2}
&\frac{\partial c}{\partial(P_{\text{Subs}}, P_B, s)}
  &&= \begin{bmatrix} 1 & 1 & 0 \\ 0 & -\Delta t & -1 \end{bmatrix}
\label{eq:kkt_cu} \\
&\frac{\partial c}{\partial (B, \tau)}
  &&= \begin{bmatrix} 0 & -p_L'(\tau) \\ 1 & 0 \end{bmatrix}
\label{eq:kkt_cx} \\
...
Let $V_B^t$ denote the gradient of the cost-to-go in the stored energy.  The
