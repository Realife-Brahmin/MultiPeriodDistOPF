\subsection{SOCP MPOPF}
\label{sec:network_mpopf}

Let $\mathcal N$, $\mathcal L$, $\mathcal B$, and $\mathcal D$ denote buses,
directed branches, battery buses, and PV buses; $0$ is the substation and
$\mathcal C(j)$ contains the children of $j$.  The network MPOPF is
...
\label{eq:net_obj}\\
\mathrm{s.t.}\quad
&P_{\mathrm{Subs}}^t-\sum_{(0,k)\in\mathcal L}P_{0k}^t=0,
\label{eq:net_sub_p}\\
&P_{\mathrm{Subs}}^t\geq0,\\
&Q_{\mathrm{Subs}}^t-\sum_{(0,k)\in\mathcal L}Q_{0k}^t=0,
\label{eq:net_sub_q}\\
