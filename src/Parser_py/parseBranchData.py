...
def parse_branch_data(system_name,
                      kVA_B=1000,
                      kV_B=2.4018,
                      Z_B=5.768643240000001,
                      verbose=False):
    ...
    # Check and compute Z_B if necessary
    if Z_B is None and kVA_B is not None and kV_B is None:
        raise ValueError("Error: You must specify both kV_B and kVA_B to calculate Z_B, or provide Z_B directly.")
    ...
    # Initialize data structures
    Nset = set()  # Set of all bus numbers
    Lset = set()  # Set of branches (edges)
    ...
    N1set = set()  # Substation node (bus 1)
    Nm1set = set()  # Buses not including substation bus (1)
    Nc1set = set()  # Buses connected to substation bus (1)
    Nnc1set = set()  # Buses not connected to substation bus (1)
    L1set = set()  # Branches where one node is the substation (1)
    Lm1set = set()  # Branches where no node is the substation (1)
        ...
        for line in file:
            ...
            if line.startswith("New Line."):
                ...
                if "Bus1" in branch_info and "Bus2" in branch_info:
                    ...
                    parent[to_bus] = from_bus
                    children.setdefault(from_bus, []).append(to_bus)
                    children.setdefault(to_bus, [])
                    parent.setdefault(from_bus, None)
