...
## Newer version of parsebatterydata
import os
import re
from collections import defaultdict
...
def parse_battery_data(system_name,
                       kVA_B=1000,
                       N_L=None,
                       verbose=False):
    # Get the working directory of this script
    ...
    # Initialize output data structures
    Bset = set()  # Set of bus numbers with batteries
    B0 = {}  # Initial SOC (kWhr) for each battery
    B0_pu = {}  # Initial SOC (kWhr) for each battery in [pu h]
    B_R = {}  # Rated storage capacity for each battery (kWhr)
    B_R_pu = {}  # Rated storage capacity for each battery [pu h]
    ...
    P_B_R_pu = {}  # Rated charging/discharging power [pu]
    S_B_R = {}  # Rated inverter apparent power (kVA)
    S_B_R_pu = {}  # Rated inverter apparent power (pu)
    Vminpu_B = {}  # Minimum voltage for each battery (pu)
    Vmaxpu_B = {}  # Maximum voltage for each battery (pu)
    soc_min = {}  # Minimum SOC (%reserve)
    soc_max = {}  # Maximum SOC (%maximum allowed, (not a real thing in OpenDSS Storage modelling))
        ...
        for line in file:
            ...
            if line.startswith("New Storage."):
                ...
                if "Bus1" in storage_info:
                    ...
                    bus = int(bus_parts[0])  # Extract the integer part
                    Bset.add(bus)
                    myprintln(verbose, f"Battery located at bus {bus}.")
                else:
                    raise ValueError("Bus1 not specified for a storage in Storage.dss")
    ...
    if N_L is None:
        ...
        Batt_percent = max(1, int((n_B / N_L) * 100))

    # Ensuring batteries always get back to their original SOCs at the end of the optimization horizon
    Bref = B0
    Bref_pu = B0_pu
    Bref_percent = {j: Bref_pu[j] / B_R_pu[j] for j in Bset}
