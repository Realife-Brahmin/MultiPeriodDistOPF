...
\title{Differential Dynamic Programming (DDP):\\Forward Pass Decomposition for Copper Plate MPOPF}
\author{}
\date{\today}
...
Differential Dynamic Programming (DDP) decomposes a multi-period optimal power flow (MPOPF) problem into $T$ sequential subproblems using a forward pass approach. Each time step $t$ solves a local optimization problem that couples with adjacent time steps through:
\begin{itemize}
