...
\title{Multi-Period Optimal Power Flow with tADMM:\\Temporally Localized ADMM for Branch Flow Model SOCP (BFM-NL)}
\author{}
\date{}
% \date{\today}
...
\section{tADMM for Branch Flow Model SOCP (BFM-NL)}
...
\begin{itemize}
    ...
    \item \textbf{Memory Efficiency}: $(3T-2) \times |\mathcal{B}|$ total storage for localized coupling
    \item \textbf{Parallel Computation}: All $T$ subproblems can be solved in parallel
    \item \textbf{Network Physics}: Branch Flow Model (BFM-NL) with SOCP relaxation for exact voltage and reactive power
\end{itemize}
...
\textbf{Spatial Network Constraints (Branch Flow Model SOCP, only for control time $t_0$):}
\begin{align}
...
\text{Power balance (nodes):} \quad & P_{ij}^{t_0} - r_{ij}\ell_{ij}^{t_0} - \sum_{(j,k) \in \mathcal{L}} P_{jk}^{t_0} = p_{L,j}^{t_0} - p_{D,j}^{t_0} - P_{B,j}^{t_0}, \quad \forall (i,j) \in \mathcal{L} \label{eq:pbal_node}\\
& Q_{ij}^{t_0} - x_{ij}\ell_{ij}^{t_0} - \sum_{(j,k) \in \mathcal{L}} Q_{jk}^{t_0} = q_{L,j}^{t_0} - q_{D,j}^{t_0}, \quad \forall (i,j) \in \mathcal{L} \label{eq:qbal_node}\\
\text{KVL (voltage drop):} \quad & v_j^{t_0} = v_i^{t_0} - 2(r_{ij} P_{ij}^{t_0} + x_{ij} Q_{ij}^{t_0}) + (r_{ij}^2+x_{ij}^2)\ell_{ij}^{t_0}, \quad \forall (i,j) \in \mathcal{L} \label{eq:kvl}\\
...
\subsection{Localized tADMM Complexity}

\textbf{For $T=48$, $|\mathcal{B}|=26$ batteries, $|\mathcal{N}|=128$ nodes, $|\mathcal{D}|=17$ PV (24-hour horizon, 30-min intervals):}
\begin{itemize}
    \item \textbf{SOC Variables per Subproblem:}
    \begin{itemize}
        \item Boundary periods (first/last, $t_0 \in \{1, 48\}$): $2 \times 26 = 52$ SOC variables
        \item Interior periods ($2 \leq t_0 \leq 47$): $3 \times 26 = 78$ SOC variables
        \item Average: $\approx 77$ SOC variables per subproblem
    \end{itemize}
    \item \textbf{Total Dual Variable Storage:}
    \begin{itemize}
        \item $(3T - 2) \times |\mathcal{B}| = (3 \times 48 - 2) \times 26 = 142 \times 26 = 3,692$ dual variables
    \end{itemize}
    \item \textbf{Communication per Iteration:}
    \begin{itemize}
        \item Each subproblem sends: $2$--$3$ local SOC values per battery to consensus
        \item Each subproblem receives: $2$--$3$ consensus SOC values per battery
        \item Total consensus exchanges: $48 \times 77 = 3,696$ SOC values per iteration
    \end{itemize}
    \item \textbf{Network Constraints per Subproblem:}
    \begin{itemize}
        \item Power balance: $128$ real + $128$ reactive = 256 constraints
        \item KVL: $127$ voltage drop constraints
        \item SOCP cones: $127$ second-order cone constraints ($\ell_{ij} \geq P_{ij}^2 + Q_{ij}^2 / v_i$)
        \item Voltage limits: $2 \times 128 = 256$ bounds ($0.95 \leq v_j \leq 1.05$ p.u.$^2$)
        \item PV reactive limits: $2 \times 17 = 34$ cone constraints ($q_D^2 \leq S_D^2 - p_D^2$)
        \item Battery SOC dynamics: $48 \times 26 = 1,248$ SOC propagation constraints
        \item Battery power limits: $2 \times 26 = 52$ bounds
    \end{itemize}
\end{itemize}
...
For BFM-NL SOCP problems (ieee123\_1ph, T=96, 26 batteries):
\begin{itemize}
    ...
    \item \textbf{Total Iterations}: Typically 100--200 iterations for tight tolerances ($\epsilon_{\text{pri}} = 10^{-5}$, $\epsilon_{\text{dual}} = 10^{-4}$) with threading enabled
\end{itemize}
...
Same as BFM-NL localized formulation:
\begin{align}
\mathcal{T}_{\text{local}}^{t_0} = \begin{cases}
\{1, 2\} & \text{if } t_0 = 1 \\
...
\midrule
Localized tADMM (BFM-NL) & 2--3 SOC + 2--3 power + network & 2--3 & $3T$ \\
\textbf{Localized Copper Plate} & \textbf{2--3 SOC + 1--2 power} & \textbf{2--3} & \textbf{3T} \\
\bottomrule
\end{tabular}
\caption{tADMM formulations with localized temporal coupling}
\end{table}
...
\section{Numerical Results and Simulation Details}

\subsection{Test System Specifications}

\textbf{Network Topology: IEEE 123-bus (Single-phase Equivalent - ieee123A\_1ph)}
\begin{itemize}
    \item \textbf{Buses}: $|\mathcal{N}| = 128$ nodes (including virtual regulator nodes)
    \item \textbf{Branches}: $|\mathcal{L}| = 127$ distribution lines
    \item \textbf{Network Type}: Radial distribution feeder
    \item \textbf{Base Voltage}: $V_{\text{base}} = 2.4018$ kV (line-to-neutral)
    \item \textbf{Base Power}: $S_{\text{base}} = 1000$ kVA
    \item \textbf{Total Load}: 1,163 kW peak demand (85 load points)
    \item \textbf{Substation}: Node 1, voltage set to 1.03 p.u. (2.474 kV)
\end{itemize}

\subsection{DER Penetration and Battery Placement}

\textbf{Photovoltaic (PV) Systems:}
\begin{itemize}
    \item \textbf{Number of PV nodes}: $|\mathcal{D}| = 17$ nodes with PV
    \item \textbf{PV Penetration}: 13.3\% of buses (17/128 nodes), 20\% of loads (17/85 loads)
    \item \textbf{Total PV Capacity}: 77.0 kW (peak rated capacity)
    \item \textbf{Individual PV Size}: 2.33--16.33 kW per installation
    \begin{itemize}
        \item Most common: 4.67 kW (11 units)
        \item Small: 2.33 kW (5 units)
        \item Large: 16.33 kW (1 unit at node 67)
    \end{itemize}
    \item \textbf{PV-to-Load Ratio}: 6.6\% (77 kW PV / 1,163 kW load)
    \item \textbf{Power Factor}: Inverter-coupled, capable of reactive power dispatch (0.85--1.0)
\end{itemize}

\textbf{Battery Energy Storage Systems (BESS):}
\begin{itemize}
    \item \textbf{Number of batteries}: $|\mathcal{B}| = 26$ battery nodes
    \item \textbf{Battery Penetration}: 20.3\% of buses (26/128 nodes), 30.6\% of loads (26/85 loads)
    \item \textbf{Co-location}: Batteries placed at load/PV buses for local energy management
    \item \textbf{Individual Battery Capacity}:
    \begin{itemize}
        \item Energy: $E_{\text{rated}}$ ranges from 9.33--98.0 kWh per battery
        \item Power: $P_{\text{rated}}$ ranges from 2.33--24.5 kW per battery
        \item Duration: Mostly 4-hour duration (typical: 18.66 kWh / 4.67 kW)
        \item SOC limits: $[20\%, 90\%]$ of rated capacity (70\% usable range)
        \item Efficiency: 95\% charge/discharge efficiency
        \item Initial SOC: 62.5\% (mid-range start)
    \end{itemize}
    \item \textbf{Total System Storage}: 527.3 kWh energy, 131.8 kW power
    \item \textbf{Battery-to-Load Ratio}: 11.3\% power (132/1,163 kW), 45.3\% energy (527/1,163 kWh)
\end{itemize}

\subsection{Temporal Resolution and Simulation Horizon}

\begin{itemize}
    \item \textbf{Time Periods}: $T = 48$ intervals (24-hour horizon)
    \item \textbf{Time Step}: $\Delta t = 0.5$ hours (30 minutes)
    \item \textbf{Load Profile}: Sinusoidal with morning/evening peaks (LoadShapeLoad)
    \begin{itemize}
        \item Base load: 80\% of peak
        \item Peak variation: $\pm$20\% sinusoidal modulation
    \end{itemize}
    \item \textbf{PV Profile}: Bell curve (active 25\%--75\% of horizon, zeros at night)
    \begin{itemize}
        \item Nighttime (0\%--25\%, 75\%--100\%): Zero generation
        \item Daytime (25\%--75\%): Half-sine wave (0 at sunrise/sunset, 1 at noon)
    \end{itemize}
    \item \textbf{Energy Prices}: Sinusoidal time-varying tariff
    \begin{itemize}
        \item Base price: \$0.08/kWh
        \item Peak surcharge: $\pm$\$0.12/kWh (range: \$0.02--\$0.20/kWh)
    \end{itemize}
\end{itemize}

\subsection{Optimization Problem Size}

\textbf{Per-Subproblem Variables (for each $t_0 \in \{1, \ldots, 48\}$):}
\begin{itemize}
    \item \textbf{Network State Variables (single time $t_0$):}
    \begin{itemize}
        \item Real power flows: $P_{ij}^{t_0}$ for $|\mathcal{L}| = 127$ branches
        \item Reactive power flows: $Q_{ij}^{t_0}$ for $|\mathcal{L}| = 127$ branches
        \item Squared voltage magnitudes: $v_j^{t_0}$ for $|\mathcal{N}| = 128$ nodes
        \item Squared current magnitudes: $\ell_{ij}^{t_0}$ for $|\mathcal{L}| = 127$ branches (SOCP)
        \item PV reactive power: $q_{D,j}^{t_0}$ for $|\mathcal{D}| = 17$ PV nodes
        \item Substation power: $P_{\text{subs}}^{t_0}, Q_{\text{subs}}^{t_0}$
        \item \textbf{Subtotal}: $127 + 127 + 128 + 127 + 17 + 2 = 528$ network variables
    \end{itemize}
    \item \textbf{Battery Variables (full horizon):}
    \begin{itemize}
        \item Battery power: $P_{B,j}^{t}$ for all $t \in \{1, \ldots, 48\}$, $j \in \mathcal{B}$ (optimized across full horizon)
        \item Local SOC trajectory: $B_j^{t_0}[t]$ for $t \in \mathcal{T}_{\text{local}}^{t_0}$ (2--3 time steps per battery)
        \item \textbf{Subtotal}: $48 \times 26 + (2$--$3) \times 26 = 1248 + 52$--$78 = 1300$--$1326$ battery variables
    \end{itemize}
    \item \textbf{Total per subproblem}: $528 + 1313 \approx 1841$ decision variables (avg)
\end{itemize}

\textbf{Global Consensus Variables:}
\begin{itemize}
    \item Battery SOC consensus: $\hat{B}_j[t]$ for all $j \in \mathcal{B}$, $t \in \mathcal{T}$
    \item \textbf{Total}: $26 \times 48 = 1,248$ consensus variables
\end{itemize}

\textbf{Total ADMM Problem:}
\begin{itemize}
    \item \textbf{Subproblems}: $T = 48$ parallel optimization problems
    \item \textbf{Total local variables}: $48 \times 1841 \approx 88,400$ variables (distributed)
    \item \textbf{Coupling variables}: $1,248$ consensus SOC variables (centralized averaging)
    \item \textbf{Dual variables}: $(3T - 2) \times |\mathcal{B}| = 142 \times 26 = 3,692$ scaled duals
\end{itemize}

\subsection{Convergence Performance}

\textbf{Typical Convergence (ieee123A\_1ph, T=48, 26 batteries):}
\begin{itemize}
    \item \textbf{Convergence Tolerances}:
    \begin{itemize}
        \item Primal tolerance: $\epsilon_{\text{pri}} = 10^{-5}$
        \item Dual tolerance: $\epsilon_{\text{dual}} = 10^{-4}$ (relaxed for faster convergence)
    \end{itemize}
    \item \textbf{Iterations to convergence}: Typically 100--200 iterations with adaptive $\rho$ and watchdog
    \item \textbf{Final primal residual}: $\|r^*\| \approx 10^{-6}$--$10^{-5}$ (satisfies $\epsilon_{\text{pri}}$)
    \item \textbf{Final dual residual}: $\|s^*\| \approx 10^{-5}$--$10^{-4}$ (satisfies $\epsilon_{\text{dual}}$)
    \item \textbf{Penalty parameter evolution}: $\rho$ typically increases from $\\approx 20,000$ to $\\approx 160,000$ in Phase 1, then stabilizes with watchdog protection
\end{itemize}

\textbf{Computational Performance:}
\begin{itemize}
    \item \textbf{Solver}: Gurobi 11.0.3 (commercial SOCP solver, NonConvex=2 mode)
    \item \textbf{Threading}: Julia multi-threading (Threads.@threads) for parallel subproblem solves
    \item \textbf{Hardware}: Typical workstation with 8--16 CPU cores
    \item \textbf{Subproblem solve time}: $\\approx 50$--$150$ ms per subproblem (SOCP with $\\approx 1,841$ variables, $\\approx 800$ constraints)
    \item \textbf{Per-iteration time}: $\\approx 100$--$300$ ms/iteration (parallel solve + consensus + dual update)
    \item \textbf{Total wall-clock time}: 15--60 seconds (48 subproblems, 100--200 iterations, parallel execution)
\end{itemize}

\textbf{Physical Results and Validation}

\textbf{Voltage Regulation:}
\begin{itemize}
    \item \textbf{Voltage limits}: $[0.95, 1.05]$ p.u. (enforced as squared: $[0.9025, 1.1025]$ p.u.$^2$)
    \item \textbf{Substation voltage}: 1.03 p.u. (2.474 kV, fixed at node 1)
    \item \textbf{Voltage drop}: Varies with load and PV injection across 128 nodes
    \item \textbf{Voltage constraint satisfaction}: All nodes within limits for optimal dispatch
\end{itemize}

\textbf{Battery and PV Operation:}
\begin{itemize}
    \item \textbf{Battery dispatch}: Charge during low-cost periods, discharge during peak price
    \item \textbf{SOC coordination}: Consensus algorithm ensures smooth SOC trajectories across all 26 batteries
    \item \textbf{PV reactive power}: Optimized within inverter capability curve ($q_D^2 + p_D^2 \leq S_D^2$)
    \item \textbf{Network losses}: Reduced through coordinated battery and PV dispatch
\end{itemize}

\textbf{Algorithm Robustness:}
\begin{itemize}
    \item \textbf{Adaptive $\rho$ with watchdog}: Two-phase strategy with hysteresis-based watchdog ensures robust convergence
    \item \textbf{Fixed $\rho$}: Requires manual tuning and often slower convergence or divergence
    \item \textbf{Phase 1 → Phase 2 transition}: Typically occurs around iteration 15--30 when $\|r\| < \epsilon_{\text{pri}}$
    \item \textbf{Watchdog activation}: Triggers when primal residual stays elevated for 20 consecutive iterations, doubling $\rho$ to enforce consensus
\end{itemize}
