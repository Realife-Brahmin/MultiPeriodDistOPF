...
function ac_power_flow_model_check(slack_bus_type,vol_ctrl_bus_type,load_bus_type,rdata_buses,rdata_loads,rdata_gens,rdata_storage,nw_pPrf_data_load,
    ...
    p_curt_pf,p_dis_pf,p_ch_pf,p_od_pf,p_ud_pf,load_theta,nd_curt_gen,nd_fl,nd_Str_active,dg_ac_pf,q_dg_pf,flex_adpf,trsf_ratio_opf_lin,flex_oltc,term_status,vm_ac,va_ac)
    ...
    vol_check_itr = zeros(size(nw_buses,1),6)
...
### ------------------- Average Scenario Generation ------------------------####
    if scenario == 0 && nTP_pf == 1                                              # Deterministic Single-Period AC-Power Flow
        ...
        # reactive_gen = rdata_gens[:,3]./sbase                                   # Q_max column in 'Gens' sheet
...
##------------------------- PV-PQ Switching ---------------------------------###
                # (nw_buses_pf,rdata_buses,nw_gens_pf,rdata_gens) = pv_pq_switching(y_bus,nw_buses_pf,nw_loads_pf,nw_gens_pf,v_mag_pf,v_angle_pf,nw_sbase_pf,rdata_buses,rdata_loads,rdata_gens,idx_oPV_buses_pf,load_bus_type,vol_ctrl_bus_type,mdf_idx_PV_buses_pf,s,t)
                ...
                if check_pv_buses_pf!=old_num_PV_buses_pf                            # Switching of PV bus to PQ has taken place
                    ...
                    reactive_gen = rdata_gens[:,3]./sbase
                    (p_sch_pf,q_sch_pf) = schd_power(nw_buses_pf,nw_gens_pf,nw_loads_pf,rdata_buses,rdata_gens,rdata_loads,nw_pPrf_data_load,nw_qPrf_data_load,active_gen,reactive_gen,p_sch_pf,q_sch_pf,idx_sch,nw_sbase_pf,t,s,p_curt_pf,p_dis_pf,p_ch_pf,p_od_pf,p_ud_pf,rdata_storage,load_theta,nd_curt_gen,nd_fl,nd_Str_active,dg_ac_pf,q_dg_pf,flex_adpf)
                    # (p_sch_pf,q_sch_pf) = schd_power(nw_buses_pf,nw_gens_pf,nw_loads_pf,rdata_buses,rdata_gens,rdata_loads,nw_pPrf_data_load,nw_qPrf_data_load,active_gen,reactive_gen,p_sch_pf,q_sch_pf,idx_sch,nw_sbase_pf,t,s,p_curt_pf,p_dis_pf,p_ch_pf,p_od_pf,p_ud_pf,rdata_storage,load_theta,nd_curt_gen,nd_fl,nd_Str_active,dg_ac_pf,q_dg_pf,flex_adpf)

                end
                old_num_PV_buses_pf = check_pv_buses_pf
# This is important. Once the buses are updated, do we have to consider the new type or not?
                # idx_PV_buses_pf  = findall(x->x==vol_ctrl_bus_type,rdata_buses[:,2])
                # idx_PQ_buses_pf  = findall(x->x==load_bus_type,rdata_buses[:,2])
####--------------------------------------------------------------------#######
                ...
                # display("Maximum mismatch P: $max_mismatch_pf_P")
                # display("Maximum mismatch Q: $max_mismatch_pf_Q")
                # for i in 1:size(nw_buses_pf,1)
                #     vol_check_itr[i,iteration] =  v_mag_pf[i,t,s]
                # end
            end # End while loop
        end     # End If condition
        # println("$v_mag_pf[i,t,s]")
...
## Print message whether the power flow algorithm is solved correctly or not ###
            ...
            if max_mismatch_pf<epsilon && iteration < itr_max
                println("The power flow has converged successfully in $iteration iterations ")
            ...
            # println("$vol_nodes_mag_pf[s,t,1]")
    end  # End Time loop
...
return vol_nodes_mag_pf,vol_nodes_theta_pf,vol_rect_pf,idx_slack_bus_pf,p_gen_node_pf,q_gen_node_pf,ac_pf_con,vol_check_itr
end
