...
cases =  {
          ...
          # "hr_dx_01_2020_brown.ods"  : [ "HR DX 01" ,
...
# LoadGrowthDict = {'pt': [1.168849815,	1.436098655,	1.704035874],
...
def  DiscipLines(LinesSheetNew, RegexMatch):
     ...
     LinesSheetNew = LinesSheetNew.sort_values('fbus')
...
#%%
NewData        = os.path.join(cases_path, r"EV-PV-Storage_Data_for_Simulations.xlsx")
...
for  key , values in cases.items(): # key = [*cases.keys()][0]; values = [*cases.values()][0]
     print('='*80,('Doing: %s' %key).center(80),sep='\n')
     ...
     for  itr, permutation in enumerate(xy):
          ...
          print('-'*60,('Doing #%4i/%-4i: %s - %s' %(itr+1, len(xy), year, loadprofilename)).center(60),'-'*60,sep='\n')
          ...
          DestinationFile= os.path.join(NewPath, PermutName+customname.upper()+'.ods')
          ...
          CaseNamesFiles.append(['_'.join([year,loadprofilename.replace('.xlsx','')]),DestinationFile])
          ...
          if True:
               ...
               RegexMatch = re.findall('.+Without_(\d+)_(\d+)',DestinationFile)
          ...
          for  Addt in ['Loads','RES','Storage']:
               ...
               AddWhere[Addt] = np.sort(np.random.choice(Candidates_List, DivideBy, p = Prob2, replace=False))
               ...
               if   Addt in 'Loads':
                    ...
                    p_profile           = FlexExcel.parse('pLoad_Profiles_Addt');
                    q_profile           = FlexExcel.parse('qLoad_Profiles_Addt')
                    ...
                    p_profile_new.iloc[FlexIDX,1:] = (             EV_weight   * ev_profile_split) + \
                                                     (LoadGrowth * Load_Weight * p_profile.iloc[FlexIDX,1:ev_profile.size+1].values)
...
CaseNamesFilesNP[:,1] = np.char.replace(CaseNamesFilesNP[:,1],'\\','/')
...
[print('(%s, %s),' %tuple(i)) for i in CaseNamesFilesNP];
