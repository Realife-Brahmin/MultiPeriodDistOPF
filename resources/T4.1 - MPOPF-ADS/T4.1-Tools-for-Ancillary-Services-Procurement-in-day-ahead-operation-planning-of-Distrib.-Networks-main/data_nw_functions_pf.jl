...
#-------------------------------------------------------------------------------
function data_lines_pf(array_lines,nw_lines,nLines)
...
function fictitious_trsf_node_pf(tap_ratio,rdata_buses,nw_lines,n_tap,tdata)
...
function data_nodes_pf(nBus,nw_buses,idx_from_line,idx_to_line,yij_line,yij_line_sh,tap_ratio,tap_ratio_min,tap_ratio_max,idx_tap,node)
    node_data  = node[]
    # tdata = []
    for i in 1:nBus
...
###############################################################################
function data_trsf_tap_pf(node_data)
...
###############################################################################
function data_trsf_tap_index_pf(tdata,itap_tr,itap_0)                                                  # Rows that has to be removed based on the idx_r values
itap_r = Float64[]
...
################################################################################
function data_load_pf(nw_loads,rheader_loads,rdata_loads,nLoads,nTP,nSc,sbase)
#--------------------------- Load Data -----------------------------------------
...
################################################################################
function data_gen_pf(nw_gens,nw_gcost,rheader_gens,rdata_gens,nGens,nTP)
...
################################################################################
function data_storage_pf(rheader_storage,rdata_storage,nw_storage,nw_Strcost,nTP,nSc,sbase)
...
return bus_data_Ssheet,bus_data_Strcost,idx_St_Strsheet,iStr_active,nStr_active,nd_Str_active,cA_str,cB_str,cC_str,prof_costA_str,prof_costB_str,prof_costC_str
end
################################################################################
function data_load_gen_pf(nLoads,nGens,nTP,nSc,bus_data_lsheet,bus_data_gsheet,Pg_max,Pg_min,Qg_max,Qg_min,nw_pPrf_data_load,nw_qPrf_data_load,nw_pPrf_data_gen_max,nw_pPrf_data_gen_min,nw_qPrf_data_gen_max,nw_qPrf_data_gen_min,nw_loads,sbase,scenario,scenario_data_p_max,scenario_data_p_min,scenario_data_q_max,scenario_data_q_min,cA_gen,cB_gen,cC_gen)
