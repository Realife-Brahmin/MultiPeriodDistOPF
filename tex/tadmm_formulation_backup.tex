...
\begin{itemize}
    ...
    \item $t_0$: Index for a specific time period in tADMM decomposition
    \item $j$: Node index
    \item $(i,j)$: Branch from node $i$ to node $j$
\end{itemize}
...
\begin{align}
\min_{\substack{P_{\text{Subs}}^{t_0}, Q_{\text{Subs}}^{t_0}, \\ P_{ij}^{t_0}, Q_{ij}^{t_0}, v_j^{t_0}, q_{D,j}^{t_0}, \\ P_{B,j}^t, \blueB{B_j^{t,t_0}} \\ \forall j \in \mathcal{B}, \, t \in \mathcal{T}}} \quad & c^{t_0} \cdot P_{\text{Subs}}^{t_0} \cdot P_{\text{BASE}} \cdot \Delta t + C_B \sum_{j \in \mathcal{B}} \left(P_{B,j}^{t_0}\right)^2 \cdot P_{\text{BASE}}^2 \cdot \Delta t \notag \\
& + \frac{\rho}{2} \sum_{j \in \mathcal{B}} \sum_{t=1}^T \left( \blueB{B_j^{t,t_0}} - \redBhat{\hat{B}_j^t} + \greenu{u_j^{t,t_0}} \right)^2
\end{align}
...
\begin{align}
\text{Real power balance (substation):} \quad & P_{\text{Subs}}^{t_0} - \sum_{j \in \text{children}(1)} P_{1j}^{t_0} = 0 \\
\text{Real power balance (nodes):} \quad & P_{ij}^{t_0} - \sum_{k \in \text{children}(j)} P_{jk}^{t_0} = P_{B,j}^{t_0} + p_{D,j}^{t_0} - p_{L,j}^{t_0}, \notag \\
& \quad \forall j \in \mathcal{N} \setminus \{1\} \\
\text{Reactive power balance (substation):} \quad & Q_{\text{Subs}}^{t_0} - \sum_{j \in \text{children}(1)} Q_{1j}^{t_0} = 0 \\
\text{Reactive power balance (nodes):} \quad & Q_{ij}^{t_0} - \sum_{k \in \text{children}(j)} Q_{jk}^{t_0} = q_{D,j}^{t_0} - q_{L,j}^{t_0}, \notag \\
& \quad \forall j \in \mathcal{N} \setminus \{1\} \\
...
\begin{align}
\text{Initial SOC:} \quad & \blueB{B_j^{1,t_0}} = B_{0,j} - P_{B,j}^1 \cdot \Delta t, \quad \forall j \in \mathcal{B} \\
\text{SOC trajectory:} \quad & \blueB{B_j^{t,t_0}} = \blueB{B_j^{t-1,t_0}} - P_{B,j}^t \cdot \Delta t, \quad \forall t \in \{2, \ldots, T\}, \, j \in \mathcal{B} \\
\text{SOC limits:} \quad & \text{SOC}_{\min,j} \cdot B_{\text{rated},j} \leq \blueB{B_j^{t,t_0}} \leq \text{SOC}_{\max,j} \cdot B_{\text{rated},j}, \notag \\
...
\begin{itemize}
    ...
    \item \textbf{Local SOC trajectory} $\blueB{B_j^{t,t_0}}$ (blue) is computed for \textit{all} time steps $t \in \{1, \ldots, T\}$
    \item The ADMM consensus penalty compares the full local trajectory $\blueB{B_j^{t,t_0}}$ with the global master copy $\redBhat{\hat{B}_j^t}$ (red)
...
\begin{align}
\redBhat{\hat{B}_j^t} &= \text{clamp}\left( \frac{1}{T} \sum_{t_0=1}^{T} \left( \blueB{B_j^{t,t_0}} + \greenu{u_j^{t,t_0}} \right), \underline{B}_j, \overline{B}_j \right)
\end{align}

where $\underline{B}_j = \text{SOC}_{\min,j} \cdot B_{\text{rated},j}$ and $\overline{B}_j = \text{SOC}_{\max,j} \cdot B_{\text{rated},j}$.

\textbf{Note:} The consensus averages over all $T$ subproblem copies of the SOC at time $t$, with projection onto the feasible SOC range.
...
\begin{align}
\greenu{u_j^{t,t_0}} &:= \greenu{u_j^{t,t_0}} + \left( \blueB{B_j^{t,t_0}} - \redBhat{\hat{B}_j^t} \right)
\end{align}
...
\begin{align}
\|r^k\|_2 &= \sqrt{\sum_{j \in \mathcal{B}} \sum_{t=1}^T \left( \frac{1}{T} \sum_{t_0=1}^T \blueB{B_j^{t,t_0}} - \redBhat{\hat{B}_j^t} \right)^2} \leq \epsilon_{\text{pri}}
\end{align}

\subsection{Dual Residual (Consensus Change)}
\begin{align}
\|s^k\|_2 &= \rho \sqrt{\sum_{j \in \mathcal{B}} \sum_{t=1}^T \left( \redBhat{\hat{B}_j^{t,k}} - \redBhat{\hat{B}_j^{t,k-1}} \right)^2} \leq \epsilon_{\text{dual}}
\end{align}

\section{Copper Plate Formulation (Simple Case)}

This section presents a simplified copper plate model (no network constraints) for reference.
